...
def quaternion_to_euler(qw, qx, qy, qz):
    ...
    myQuaternion = Quaternion([qw, qx, qy, qz])
    roll = myQuaternion.degrees
...
def read_position(filename): 
    ...
    with open (filename, 'r', newline='') as csvfile:
        ...
        x_indix = header.index(x_colum)
        y_indix = header.index(y_colum)
        z_indix = header.index(z_colum)

        for row in csvreader:
            x_positions.append(float(row[x_indix]))
            y_positions.append(float(row[y_indix]))
            z_positions.append(float(row[z_indix]))
